...
curl -fsS -X POST "$BASE_URL/tools/recommend_capabilities_for_role" \
  ...
  --data '{"tenantId":"tenant-contoso","projectId":"renewals-hackathon","role":"Enterprise Account Manager","department":"Customer Success","workSignalSummary":"Synthetic summaries only.","maxResults":2,"correlationId":"local-smoke-read"}' \
  >/dev/null
...
status="$(curl -sS -o /tmp/signal-foundry-local-unauthorized.json -w "%{http_code}" \
  ...
  -H "content-type: application/json" \
  -H "x-sf-actor-id: actor-priya" \
  --data '{"tenantId":"tenant-contoso","projectId":"renewals-hackathon","idempotencyKey":"local-smoke-bad-approve","proposalId":"prop-missing","reviewer":"Priya Shah","approvalNotes":"Unauthorized smoke.","confirmed":true}')"
